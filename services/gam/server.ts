...
export function createApp() {
  return createServer({
    registerRoutes(app) {
      // GET /me/gam/weekly - GAM weekly analytics for user
      app.get<{
        Querystring: WeeklyUserQuery;
      }>('/me/gam/weekly', async (req, reply) => {
        ...
        try {
          ...
          app.log.error(error);
          reply.code(500).send({ ok: false, error: 'Failed to fetch GAM data' });
        }
      });

      // GET /org/:orgId/gam/weekly - GAM weekly analytics for organization
      app.get<{
        Params: { orgId: string };
        Querystring: WeeklyOrgQuery;
      }>('/org/:orgId/gam/weekly', async (req, reply) => {
        ...
        try {
          ...
          app.log.error(error);
          reply.code(500).send({ ok: false, error: 'Failed to fetch org GAM data' });
        }
      });
    },
  });
