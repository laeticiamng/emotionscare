  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      toast.success("Redirection vers le paiement...", {
        description: "Votre panier a été créé avec succès ✨"
      });
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative" aria-label={`Panier${totalItems > 0 ? ` - ${totalItems} article${totalItems > 1 ? 's' : ''}` : ''}`}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle>Panier EmotionsCare</SheetTitle>
          <SheetDescription>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Explorez notre collection de produits bien-être
                </p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2 bg-card/50 rounded-lg">
