...
export const retentionService = {
    ...
    try {
      ...
      for (let w = weeks; w >= 0; w--) {
        ...
        for (let followupWeek = 0; followupWeek <= w; followupWeek++) {
          ...
          const activeUserIds = new Set((activeSessions as any[])?.map(s => s.user_id) || []);
  ...
  async getStreaksDistribution(): Promise<Array<{
    ...
    try {
      ...
      const distribution = ranges.map(range => {
        const count = (profiles as any[]).filter(p => {
  ...
  async createCampaign(campaign: {
    ...
    try {
      ...
      return {
        ...
        target: (data as any).target_audience,
  ...
  async getTopEngagedUsers(limit: number = 20): Promise<Array<{
    ...
    try {
      ...
      return (profiles as any[]).slice(0, limit).map(p => ({
        userId: p.id,
  ...
  async calculateAverageLTV(): Promise<{
    ...
    try {
      ...
      const now = Date.now();
      const totalDays = (profiles as any[]).reduce((sum, p) => {
