...
/**
 * Helper to map a Supabase playlist row to a Playlist object
 */
function mapRow(row: any): Playlist {
  return {
    id: row.id,
    name: row.name,
    title: row.name,
    tracks: (row.tracks || []).map((t: any) => ({
      ...t,
      url: t.url || t.audioUrl || t.audio_url || '',
    })),
  };
}
...
export const getRecommendedPlaylists = async (emotion: string): Promise<Playlist[]> => {
  try {
    ...
    const { data, error } = await supabase
      .from('music_playlists')
      .select('*')
      .ilike('emotion', `%${emotion.toLowerCase()}%`)
      .order('created_at', { ascending: false });
