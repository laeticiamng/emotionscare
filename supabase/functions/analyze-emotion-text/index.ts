...
serve(async (req) => {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error in analyze-emotion-text';
    const errorDetails = error instanceof Error ? error.stack : String(error);
    console.error('Error in analyze-emotion-text function:', errorMessage, errorDetails);
    return new Response(JSON.stringify({ 
      error: errorMessage,
      success: false 
    }), {
