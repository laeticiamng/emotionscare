...
export default function B2BTeamMonthlyReportPage({ params }: TeamReportPageProps) {
  const { period, team } = params;
  ...
  useEffect(() => {
    let isMounted = true;
    setStatus('loading');
    fetchMonthlyReport(period, team)
      .then((payload) => {
        if (!isMounted) return;
        setReport(payload);
        setStatus('idle');
      ...
      .catch((error) => {
        console.error('b2b.team-report.fetch.failed', error);
        if (!isMounted) return;
        setStatus('error');
      });
    return () => {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const payload = await exportMonthlyReportCsv(period, team);
      if (payload.url) {
      ...
      if (payload.fallback?.csv) {
        ...
        link.href = url;
        link.download = `rapport-${period}-${team}.csv`;
      ...
      console.error('b2b.team-report.export.error', error);
    } finally {
      setExporting(false);
    }
  };
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-6 p-6" aria-live="polite">
      {status === 'loading' && (
        <p className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-500">Chargement du rapport équipe…</p>
      )}

      {status === 'error' && (
        <p className="rounded-lg border border-rose-100 bg-rose-50 p-4 text-sm text-rose-600">
          Rapport équipe indisponible pour cette période.
        </p>
      )}
