...
test.describe('Authentication Flows E2E', () => {
  ...
  test('B2C authentication complete flow', async ({ page }) => {
    ...
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2C_LOGIN);

    // Test registration flow
    await page.click('text=Créer un compte');
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2C_REGISTER);
  ...
  test('B2B user authentication flow', async ({ page }) => {
    ...
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2B_SELECTION);

    // Select user option
    await page.click('text=Collaborateur');
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2B_USER_LOGIN);
    ...
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2B_USER_DASHBOARD);
  ...
  test('B2B admin authentication flow', async ({ page }) => {
    ...
    await expect(page).toHaveURL(UNIFIED_ROUTES.B2B_ADMIN_LOGIN);

    await page.fill('input[name="email"]', 'admin@company.com');
    await page.fill('input[name="password"]', 'admin123');
    await page.click('button[type="submit"]');

    await expect(page).toHaveURL(UNIFIED_ROUTES.B2B_ADMIN_DASHBOARD);
  ...
  test('logout flow', async ({ page }) => {
    ...
    await expect(page).toHaveURL(UNIFIED_ROUTES.HOME);
