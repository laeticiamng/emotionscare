...
const DISCLAIMERS = [
  ...
  "En cas de danger immédiat ou de détresse, contactez les services d'urgence (112 en Europe).",
  "Vos échanges sont anonymisés et destinés à un accompagnement confidentiel."
];
...
serve(async (req) => {
  ...
  try {
    ...
    if (authResult.status !== 200 || !authResult.user) {
      console.warn('[ai-coach] Unauthorized access attempt');
      return createErrorResponse(authResult.error || 'Authentication required', authResult.status, corsHeaders);
    }

    // 🛡️ SÉCURITÉ: Rate limiting strict (5 req/min)
    const rateLimit = await enforceEdgeRateLimit(req, {
      route: 'ai-coach',
      userId: authResult.user.id,
      limit: 5,
      windowMs: 60_000,
      description: 'AI Coach - Lovable AI calls'
    });

    if (!rateLimit.allowed) {
      console.warn('[ai-coach] Rate limit exceeded', { userId: authResult.user.id });
    ...
    console.log('🧠 AI Coach - Génération de réponse:', {
      userEmotion,
      coachPersonality,
    ...
    const systemPrompt = `Tu es un expert coach en bien-être mental EmotionsCare avec une formation en psychologie positive et thérapie ACT.
...
- Oriente vers un professionnel si besoin urgent
- Garde tes réponses concises (2-3 phrases max, moins de 80 mots)
- Suggère des ressources de l'app: respiration (/app/breath), journal (/app/journal), musique (/app/music)
- Si détresse grave détectée, encourage à contacter le 112 ou un proche de confiance

Tu dois répondre en JSON valide avec cette structure exacte:
{
  "response": "Ta réponse empathique ici (max 80 mots)",
  "emotion": "supportive|encouraging|calm|energizing",
  "techniques": ["technique 1", "technique 2", "technique 3"],
  "resources": [{"type": "méditation|exercice|musique|respiration", "title": "Titre", "description": "Description courte"}],
...
Génère ta réponse JSON:`;
    ...
    // Add suggestions and disclaimers for compatibility with CoachView
    coachResponse.suggestions = coachResponse.techniques;
    coachResponse.disclaimers = DISCLAIMERS;
    ...
    const fallbackResponse: CoachResponse = {
      ...
      suggestions: ["Respiration", "Ancrage", "Auto-compassion"],
