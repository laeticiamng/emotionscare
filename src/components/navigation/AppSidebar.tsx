...
const AppSidebar = () => {
  ...
  const userRole = (user as any)?.role || user?.user_metadata?.role || 'b2c';

  // Navigation selon le rôle
  const getMainNavigation = (): NavItem[] => {
    const baseItems: NavItem[] = [
      { title: 'Accueil', url: userRole === 'b2c' ? '/app/home' : userRole === 'b2b_user' ? '/app/collab' : '/app/rh', icon: Home }
    ];
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {userRole === 'b2c' && 'Particulier'}
                  {userRole === 'b2b_user' && 'Employé'}
                  {userRole === 'b2b_rh' && 'RH Manager'}
                </div>
              </div>
            </div>
          </div>
