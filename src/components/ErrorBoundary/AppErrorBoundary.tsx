...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  public render(): ReactNode {
    if (this.state.hasError) {
      ...
      return (
        <div className={`flex items-center justify-center p-4 ${isComponentLevel ? 'min-h-[200px]' : 'min-h-screen'} bg-background`}>
          <Card className="max-w-md w-full border-red-200 bg-red-50 dark:bg-red-900/20">
            <CardHeader>
              <CardTitle className="text-red-800 dark:text-red-200 flex items-center gap-2">
                <AlertTriangle className="h-5 w-5" />
                {isComponentLevel ? 'Erreur de composant' : 'Une erreur inattendue s\'est produite'}
              </CardTitle>
              <CardDescription className="text-red-600 dark:text-red-300">
                {isComponentLevel 
                  ? 'Ce composant a rencontré un problème.'
                  : 'L\'application a rencontré un problème. Vous pouvez essayer de recharger la page ou revenir à l\'accueil.'
                }
              </CardDescription>
            </CardHeader>
