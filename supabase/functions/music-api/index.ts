 ...
 *
 * 🔒 SÉCURISÉ: Auth + Rate limit 40/min + CORS restrictif
 *
 * Endpoints:
 * Sessions:
...
serve(async (req) => {
  ...
  if (!corsResult.allowed) {
    console.warn('[music-api] CORS rejected - origin not allowed');
  ...
  try {
    ...
    if (!user) {
      console.warn('[music-api] Unauthorized access attempt');
    ...
    const rateLimit = await enforceEdgeRateLimit(req, {
      ...
      limit: 40,
      windowMs: 60_000,
      description: 'Music API - Music therapy sessions and playlists',
    });

    if (!rateLimit.allowed) {
      console.warn('[music-api] Rate limit exceeded', { userId: user.id });
    ...
    console.log(`[music-api] Processing for user: ${user.id}`);
